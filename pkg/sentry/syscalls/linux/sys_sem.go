...
func Semctl(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	case linux.GETZCNT:
		v, err := fetchSemzcnt(t, id, num)
		return uintptr(v), nil, err

	case linux.GETNCNT:
		v, err := fetchSemncnt(t, id, num)
		return uintptr(v), nil, err
...
func fetchSemzcnt(t *kernel.Task, id int32, num int32) (uint16, error) {
	r := t.IPCNamespace().SemaphoreRegistry()
	set := r.FindByID(id)
	if set == nil {
		return 0, syserror.EINVAL
...
func fetchSemncnt(t *kernel.Task, id int32, num int32) (uint16, error) {
	r := t.IPCNamespace().SemaphoreRegistry()
	set := r.FindByID(id)
	if set == nil {
		return 0, syserror.EINVAL
