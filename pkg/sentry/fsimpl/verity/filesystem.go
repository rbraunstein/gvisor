...
func (fs *filesystem) verifyStatAndChildren(ctx context.Context, d *dentry, stat linux.Statx) error {
	...
	if d.isDir() && len(d.childrenNames) == 0 {
		...
		childrenNames, err := fd.GetXattr(ctx, &vfs.GetXattrOptions{
			Name: childrenXattr,
			Size: uint64(childrenSize),
		})
		if err == syserror.ENODATA {
			return alertIntegrityViolation(fmt.Sprintf("Failed to get xattr %s for merkle file of %s: %v", childrenXattr, childPath, err))
		}
		if err != nil {
			return err
		}
...
func (fs *filesystem) lookupAndVerifyLocked(ctx context.Context, parent *dentry, name string) (*dentry, error) {
	...
	child.uid = stat.UID
	child.gid = stat.GID
	child.childrenNames = make(map[string]bool)
